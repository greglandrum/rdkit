...
void testCreateQueryAtoms() {
  BOOST_LOG(rdInfoLog) << "-------------------------------------" << std::endl;
  BOOST_LOG(rdInfoLog)
      << "Testing creation of query atoms that can be converted to SMARTS."
      << std::endl;
  {  // aromatic atom
    QueryAtom at(6);
    at.setIsAromatic(true);
    at.expandQuery(makeAtomAromaticQuery(), Queries::COMPOSITE_AND);

    std::string sma = SmartsWrite::GetAtomSmarts(&at);
    // std::cerr << sma << std::endl;
    TEST_ASSERT(sma == "c");

    // atom map
    at.setAtomMapNum(4);
    sma = SmartsWrite::GetAtomSmarts(&at);
    // std::cerr << sma << std::endl;
    TEST_ASSERT(sma == "[c:4]");
  }
  {  // aliphatic atom
    QueryAtom at(6);
    at.expandQuery(makeAtomAliphaticQuery(), Queries::COMPOSITE_AND);
  ...
  {  // atom list
    ...
    {
      QueryAtom tat(6);
      tat.expandQuery(makeAtomAromaticQuery(), Queries::COMPOSITE_AND);
      at.expandQuery(tat.getQuery()->copy(), Queries::COMPOSITE_OR);
    }
    {
      QueryAtom tat(7);
      tat.expandQuery(makeAtomAliphaticQuery(), Queries::COMPOSITE_AND);
      at.expandQuery(tat.getQuery()->copy(), Queries::COMPOSITE_OR);
    }
    {
      QueryAtom tat(8);
  ...
  {  // basic bond queries
    ...
    RWMol m;
    bool updateLabel = false, takeOwnership = false;
    m.addAtom(&at, updateLabel, takeOwnership);
    m.addAtom(&at, updateLabel, takeOwnership);

    QueryBond bnd(Bond::AROMATIC);
    bnd.setBeginAtomIdx(0);
    bnd.setEndAtomIdx(1);
    m.addBond(&bnd, false);
  ...
  {  // basic bond queries
    ...
    QueryBond bnd(Bond::SINGLE);
    bnd.expandQuery(makeBondOrderEqualsQuery(Bond::AROMATIC),
