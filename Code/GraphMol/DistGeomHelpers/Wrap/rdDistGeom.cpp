...
struct PyEmbedParameters
 ...
 public:
  ...
  void setExplicitTorsions(const python::tuple &torsions) {
    explicitTorsions.clear();
    unsigned int nTors =
        python::extract<unsigned int>(torsions.attr("__len__")());

    for (unsigned int i = 0; i < nTors; ++i) {
      python::tuple aids = python::extract<python::tuple>(torsions[i][0]);
      python::tuple vs = python::extract<python::tuple>(torsions[i][1]);
      python::tuple signs = python::extract<python::tuple>(torsions[i][2]);
      RDKit::DGeomHelpers::UserProvidedTorsion ut;
      pythonObjectToVect(aids, ut.atoms);
      pythonObjectToVect(vs, ut.V);
      pythonObjectToVect(signs, ut.signs);
      explicitTorsions.push_back(ut);
    }
  }
...
\n";
  ...
  python::class_<PyEmbedParameters, boost::noncopyable>(
      ...
      .def("SetExplicitTorsions", &PyEmbedParameters::setExplicitTorsions,
           python::args("self", "torsions"), "docs coming");
